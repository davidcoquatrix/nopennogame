...
### Fixed
- [x] **Editing a saved custom game created a duplicate instead of updating it.** `CustomGame/Setup.razor.CreateGame()` always called `new GameTemplate(Guid.NewGuid(), ...)`, even when reached via the edit flow (`Settings.razor` → "✎" → `custom-game?baseGameId=...`). `LocalStorageGameCatalogueService.SaveCustomGameAsync`'s upsert matches by `Id`, so a fresh Id never matched the original entry — every edit (with "Sauvegarder ce modèle" checked) added a new catalogue entry and left the old one orphaned.
  - *Fixed by reusing `BaseGameId` as the template's `Id` when editing (`templateId = BaseGameId ?? Guid.NewGuid()`), so the upsert now correctly replaces the existing catalogue entry instead of adding a second one. `BaseGameId` is only ever set from the edit flow (the "Jeu personnalisé" creation tile navigates to `/custom-game` with no query param), so this is safe — never accidentally overwrites a built-in game.*
  - *Also added a duplicate-name guard on `CreateGame()` (case-insensitive, excluding the game being edited) so two custom games can no longer be created with the same name in the first place — mirrors the existing player-name duplicate check in `Players/Setup.razor`.*
